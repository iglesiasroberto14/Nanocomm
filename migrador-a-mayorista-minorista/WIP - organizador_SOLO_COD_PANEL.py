...
#        domcodpanel_len = int(len(minorista_domcodpanel)/4)
        ...
        while i <= domid_len:
            ...
            concat=minorista_domid[i*4-3:i*4-1]+minorista_receptor+minorista_linea
            idcom_idmin[concat]=minorista_id
            print("Aca: "+concat+','+str(idcom_idmin[concat]))
            #j += 1
            i += 1
        idcom_idmin[minorista_id]=[dom_id]
...
#        else:
        try:
            cuentas_ok.write(str(idcom_idmin[comunicador["idComunicador"][:2]+comunicador["receptorTel"]+comunicador["lineaTel"]])+','+comunicador["idComunicador"]+'\n')
            j+=1
        except:
            print('3No encontre la combinacion de dominios, receptora y linea: ' + comunicador["idComunicador"][:2]+ comunicador["receptorTel"] + comunicador["lineaTel"] +". Cuenta: "+comunicador["idComunicador"])
